...
class Characteristic(dbus.service.Object):
    ...
    def WriteValue(self, value, options):
        ...
        # Tentar extrair MAC do device path
        device_mac = "UNKNOWN"
        ...
        if self.callback:
            # Chama o callback protegendo contra erros de argumentos
            try: self.callback(data_bytes, device_mac)
            except TypeError: self.callback(data_bytes)
        else:
            print(f"[GATT] Recebido {len(value)} bytes de {device_mac}")
    ...
    def StartNotify(self):
        self.notifying = True
        # print("[GATT] Notificações ATIVADAS")
    ...
    def SendNotification(self, data_bytes):
        # FIX CRÍTICO: Comentamos a proteção 'if not self.notifying'.
        # Isto garante que enviamos o PropertiesChanged mesmo que o BlueZ
        # ainda não tenha chamado StartNotify (comum em handshakes rápidos).
...
class GATTServerManager:
    def __init__(self, bus, adapter_index=0):
        ...
        try:
            self.service_manager = dbus.Interface(bus.get_object('org.bluez', adapter_path), 'org.bluez.GattManager1')
        except Exception as e:
            print(f"[GATT] ERRO: Adaptador {adapter_path} inacessível: {e}")

        # Monitor de desconexão
        try:
            self.bus.add_signal_receiver(self._on_properties_changed, dbus_interface='org.freedesktop.DBus.Properties', signal_name='PropertiesChanged', path_keyword='path')
        except: pass
    ...
    def _on_rx_data(self, data_bytes, device_mac="UNKNOWN"):
        if device_mac != "UNKNOWN": self._seen_clients.add(device_mac)
        if self._data_callback:
            try: self._data_callback(data_bytes, device_mac)
            except TypeError: self._data_callback(data_bytes)

    def set_disconnect_callback(self, callback):
        self._disconnect_callback = callback

    def _on_properties_changed(self, interface, changed, invalidated, path=None):
        if interface != 'org.bluez.Device1' or 'Connected' not in changed: return
        if changed['Connected']: return
        
        # Lógica para extrair MAC e avisar app
        try:
            mac = str(path).split('dev_')[-1].replace('_', ':')
            if mac in self._seen_clients and self._disconnect_callback:
                self._disconnect_callback(mac)
        except: pass
    ...
    def register(self):
        try:
            self.service_manager.RegisterApplication(self.app.get_path(), {}, reply_handler=self.register_callback, error_handler=self.register_error_callback)
        except Exception as e: print(f"[GATT] Falha registo: {e}")

    def register_callback(self): print("[GATT] Serviço SIC Registado.")
    def register_error_callback(self, error): print(f"[GATT] Erro registo: {error}")