...
class NodeApp:
    def __init__(self):
        ...
        # 5. Heartbeat
        self.hb_monitor = HeartbeatManager(self.manager, timeout_seconds=6)
    ...
    def setup_gatt(self):
        def run_gatt_server():
            try:
                ...
                advertiser = NodeAdvertiser(MY_NID, hops=1)
    ...
    def print_menu(self):
        ...
        print("3. Connect <index> (Connect to scanned device)")
    ...
    def print_status(self):
        ...
        # Downlinks
        # router.downlink_keys tem MACs. Precisamos NIDs da forwarding_table
        downlinks = []
        ...
        while self.running:
            try:
                ...
                if cmd == '1' or cmd == 'status':
                    ...
                    print("[SCAN] Scanning...")
                    self.candidates = self.manager.adapter.scan_for(3000) # Raw scan or use manager helper
                    # Usar helper do manager se acessível, ou implementar aqui
                    # manager.find_and_connect é automático.
                    # Vamos usar scan_for_candidates de common/ble_layer se possível
                    # Mas manager encapsula. Vamos chamar o método estático ou raw.
                    ...
                    if len(cmd_line) < 2:
                        print("Use: connect <index>")
                        continue
                    try:
                        idx = int(cmd_line[1])
                        if 0 <= idx < len(self.candidates):
                            cand = self.candidates[idx]
                            print(f"[CLI] Connecting to {cand['name']}...")
                            # Manual connect logic using Manager internals (hacky but functional for CLI)
                            ...
                            device.connect()
                            time.sleep(2)
                            # Setup (Service check, notify, etc - copied from manager logic)
                            # Simplificação: Chamar manager._setup_uplink(device)? Não existe.
                            # Vamos confiar no manager.find_and_connect_uplink para automação
                            # ou reimplementar setup mínimo aqui.
                            # Para garantir consistência, o melhor seria o Manager ter `connect_to(device)`.
                            # Como não tem, e o código é complexo, sugerimos usar o automágico:
                            print("[CLI] Triggering automatic connection logic...")
                    ...
                    if SINK_NID in self.dtls_manager.sessions:
                        ...
                        print("[CLI] No Secure Session with Sink yet.")
